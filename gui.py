...
config_template = '''# Lucrum related configs
LUCRUM_API_KEY = '{0}'
LUCRUM_API_SECRET = '{1}'
LUCRUM_URL = '{2}'
...
class BiometricWindow(QMainWindow):
    ...
    def setup_window(self):
        ...
        self.setWindowTitle('Lucrum Biometric Service')

    def setup_textboxes_and_label(self):

        self.create_label("API Secret", "api_secret", 20, 0, 200, 30)
        self.create_field("textbox_lucrum_api_secret", 20, 30, 200, 30)

        self.create_label("API Key", "api_key", 20, 60, 200, 30)
        self.create_field("textbox_lucrum_api_key", 20, 90, 200, 30)

        self.create_label("Lucrum URL", "lucrum_url", 20, 120, 200, 30)
        self.create_field("textbox_lucrum_url", 20, 150, 200, 30)
    ...
    def set_default_value_or_placeholder_of_field(self):
        if os.path.exists("local_config.py"):
            import local_config as config
            self.textbox_lucrum_api_secret.setText(config.LUCRUM_API_SECRET)
            self.textbox_lucrum_api_key.setText(config.LUCRUM_API_KEY)
            self.textbox_lucrum_url.setText(config.LUCRUM_URL)
            ...
            self.textbox_lucrum_api_secret.setPlaceholderText("c70ee57c7b3124c")
            self.textbox_lucrum_api_key.setPlaceholderText("fb37y8fd4uh8ac")
            self.textbox_lucrum_url.setPlaceholderText("example.lucrum.com")
    ...
    def integrate_biometric(self):
        ...
        if not hasattr(self, 'p'):
            ...
            command = shlex.split('python -c "from lucrum_sync import infinite_loop; infinite_loop()"')
    ...
    def get_local_config(self):
        ...
        return config_template.format(self.textbox_lucrum_api_key.text(), self.textbox_lucrum_api_secret.text(), self.textbox_lucrum_url.text(), self.textbox_pull_frequency.text(), formated_date, json.dumps(devices), json.dumps(shifts))
...
def validate_fields(self):
    ...
    if not self.textbox_lucrum_api_key.text():
        return message("API Key")

    if not self.textbox_lucrum_api_secret.text():
        return message("API Secret")

    if not self.textbox_lucrum_url.text():
        return message("Lucrum URL")
