...
DUPLICATE_LOG_ENTRY = "This employee already has a log with the same timestamp"
...
def pull_process_and_push_data(device, device_attendance_logs=None):
    ...
    for device_attendance_log in device_attendance_logs[index_of_last+1:]:
        ...
        lucrum_status_code, lucrum_message = send_to_lucrum(device_attendance_log['user_id'], device_attendance_log['timestamp'], device['device_id'], punch_direction)
        if lucrum_status_code == 200:
            attendance_success_logger.info("\t".join([lucrum_message, str(device_attendance_log['uid']),
            ...
            attendance_failed_logger.error("\t".join([str(lucrum_status_code), str(device_attendance_log['uid']),
                str(device_attendance_log['user_id']), str(device_attendance_log['timestamp'].timestamp()),
                str(device_attendance_log['punch']), str(device_attendance_log['status']),
                json.dumps(device_attendance_log, default=str)]))
...
def send_to_lucrum(employee_field_value, timestamp, device_id=None, log_type=None):
    """
    Example: send_to_lucrum('12349',datetime.datetime.now(),'HO1','IN')
    """
    url = config.LUCRUM_URL + "/api/method/lucrum.hr.doctype.employee_checkin.employee_checkin.add_log_based_on_employee_field"
    headers = {
    ...
    if response.status_code == 200:
        ...
        if EMPLOYEE_NOT_FOUND_ERROR_MESSAGE in error_str:
            error_logger.error('\t'.join(['Error during Lucrum API Call.', str(employee_field_value), str(timestamp.timestamp()), str(device_id), str(log_type), error_str]))
            # TODO: send email?
        else:
            error_logger.error('\t'.join(['Error during Lucrum API Call.', str(employee_field_value), str(timestamp.timestamp()), str(device_id), str(log_type), error_str]))
            ...
            for shift in shift_type_device_map['shift_type_name']:
                try:
                    ...
                    if (sync_current_timestamp and min_pull_timestamp > sync_current_timestamp) or (min_pull_timestamp and not sync_current_timestamp):
                        response_code = send_shift_sync_to_lucrum(shift, min_pull_timestamp)
...
def send_shift_sync_to_lucrum(shift_type_name, sync_timestamp):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            error_str = _safe_get_error_str(response)
            error_logger.error('\t'.join(['Error during Lucrum Shift Type API Call.', str(shift_type_name), str(sync_timestamp.timestamp()), error_str]))
